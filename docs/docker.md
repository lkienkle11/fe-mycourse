...
- **Quality gates:** CI runs **`npm run test-all`** (Knip: unused types + component/screen files) before build; locally run **`npm run check-all`** before shipping. The Docker image only runs the **production build** step (same as CI **`build`** / VPS deploy).
...
- [`docs/quality.md`](quality.md) — `test-all`, `check-all`, `deadcode` (Knip), lint/build gates
