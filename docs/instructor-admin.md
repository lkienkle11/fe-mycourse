...
| Component | Purpose |
|-----------|---------|
| `InstructorProfileViewDialog` | Read-only profile popup (approvals / roster / profiles). Title uses applicant `display_name` (merge list row + detail). Field order: headline → bio → **CV** (`PreviewPdf`) → years of experience → …; certificates via **`InstructorCertificateCarousel`** (one slide per cert, horizontal nav) |
...
| ADM-02 | `PreviewPdf` + `InstructorProfileViewDialog` — company snapshot, topics/skills chips, CV + certificate PDF inline. **CV** block is directly after **Giới thiệu** (`bio`), before **Số năm kinh nghiệm**. **Certificates** render in a horizontal **carousel** (`InstructorCertificateCarousel`, Embla via `src/components/ui/carousel.tsx`) — one certificate per slide; prev/next when >1; credential URL centered on the slide when present. Dialog title `Đơn — {name}` uses `mergeInstructorApplicationDetail(listRow, detail)` so identity from the list row is kept when detail payload lacks `display_name`. Dialog width: `w-[calc(100%-2rem)] sm:max-w-3xl lg:max-w-4xl`. Approvals / profiles may fetch `GET …/:id` for hydrated media |
