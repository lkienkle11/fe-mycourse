...
- **`src/components/shared/delta-editor.tsx`** — WYSIWYG `DeltaEditor` + read-only `DeltaViewer` for Quill Delta JSON (optional font family picker unless `lockSystemFont`; optional hyperlink toolbar via `allowLink` + `DeltaEditorLinkDialog` — text and **image embed** links, **link text color** picker via `bindQuillLinkColorHandler` in `delta-editor-link-color.ts`, Quill Snow tooltip **Edit/Remove** via `bindQuillLinkTooltipFix` in `delta-editor-link-quill.ts`, **image embed 4-corner drag-resize** via `bindQuillImageResize` in `delta-editor-image-resize.ts`, embed × remove, `onObjectEmbedded` / `onDelete` callbacks, `MediaCollectionDialog` toolbar embeds). Instructor `bio` uses restricted mode: `allowMediaEmbed={false}`, `lockSystemFont` (no font picker, no heading picker; `font`/`size`/`header` stripped via `TEXT_DELTA_LOCKED_FORMAT_ATTRIBUTES`), clipboard blocks IMG/VIDEO. Quill blot/helpers in `src/lib/quill/` (`delta-editor-quill.ts`, `delta-editor-link-quill.ts`, `delta-editor-link-color.ts`, `delta-editor.css`).
