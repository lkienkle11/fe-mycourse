...
- **UI**:
  ...
  - `src/components/shared/delta-editor.tsx` — shared WYSIWYG `DeltaEditor` + read-only `DeltaViewer` (Delta JSON; font + heading pickers omitted when `lockSystemFont`; inline image/video via toolbar, paste, or drag-and-drop when media allowed; embed × remove; `onObjectEmbedded` / `onDelete`). Instructor bio uses restricted `DeltaEditor` (`allowMediaEmbed={false}`, `lockSystemFont` — strips `font`/`size`/`header`); admin dialog renders bio with the plain `DeltaViewer` (`value`/`className` only — no restriction props; it displays Delta already allow-listed by the BE).
  ...
  - `src/lib/utils/course-delta.ts` — shared Delta parse/stringify/text/embed-diff helpers for validation and editor state (`countDeltaCodePoints` + `sanitizeLockedTextDelta` + `TEXT_DELTA_LOCKED_FORMAT_ATTRIBUTES` for instructor bio; `coerceToDelta` drops malformed legacy ops)
