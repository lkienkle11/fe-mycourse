...
- **Purpose**: Shared multi-select user picker UI. `UserMultiSelectPickerFeatureDialog` wires picker state + SWR candidates + labels (dedupes feature wrappers). `onAfterPartialSuccess` revalidates candidate list after bulk add (full success before close, or partial success while dialog stays open). `USER_PICKER_PER_PAGE` in `src/constants/user-picker.ts`. Bulk confirm handlers share `finalizeBulkUserPickerSubmit` (`src/lib/utils/user-picker-bulk-submit.ts`) for partial-success toasts + `UserPickerConfirmResult`.
...
### Asset: bulk user-picker partial-success submit
- **Name**: `finalizeBulkUserPickerSubmit`, `BulkUserPickerSubmitToasts`
- **Type**: Utility function
- **Path**: `src/lib/utils/user-picker-bulk-submit.ts`
- **Purpose**: Shared confirm-handler flow for bulk user-picker modals — inject bulk API call + toast callbacks + `mapSucceededIds`; handles all-success / all-failed / partial-success; suppresses duplicate API error toast when all picks fail (internal sentinel). Optional `afterSubmit` runs only when at least one pick succeeded (e.g. roster SWR `mutate()` — skipped on all-failed).
- **Scope**: Course collaborator add (`useCourseCollaboratorActions`) + instructor roster add (`InstructorRosterPage.handleAddInstructors`).
- **Dependencies**: `UserPickerConfirmResult`, `toastApiError`, feature-specific bulk services.
...
- **Purpose**: Paginated collaborator list with search/URL sync; owner opens multi-select picker backed by `GET …/instructor-candidates` (`course_collaborator_candidate:read` / P67, replaces global `instructor_roster:read` P41). Picker stays open on add failure (selection preserved for retry). `useCourseCollaboratorActions` submits `role: "EDITOR"`; optional `AddCollaboratorsBulkPayload.role` accepts only `EDITOR`, never `OWNER`, while response `CourseCollaboratorRole` remains `OWNER | EDITOR` for display. After remove on the last page, tab clamps URL to `total_pages`. List refresh uses `useCourseCollaborators().mutate()` only — add/remove do not refetch full `CourseDetail` (submit revalidates via `mutateDetail()`). **Responsive overflow**: picker `max-w-xl` with `overflow-x-hidden`; long `display_name` uses `truncate` + `title`; long `email` uses `break-all` (same on tab list rows). Pattern matches `course-editor-dialogs` (`min-w-0` flex chain). UI gates add/picker with `PermissionGate` + `canManageCollaborators` (OWNER).
...
- **Name**: `createEmptyDelta`, `createEmptyDeltaString`, `parseDelta`, `coerceToDelta`, `stringifyDelta`, `extractPlainText`, `extractDeltaPreviewText`, `stripMediaEmbedsFromDelta`, `stripDeltaFormatAttributes`, `extractMediaEmbedsFromDelta`, `diffRemovedMediaEmbeds`, `countDeltaNonWhitespace`, `countDeltaCodePoints`, `sanitizeLockedTextDelta`, `TEXT_DELTA_LOCKED_FORMAT_ATTRIBUTES`
- **Type**: Utility functions
- **Path**: `src/lib/utils/course-delta.ts`
- **Purpose**: Shared Quill-Delta parsing/stringify/text extraction, legacy plain-text coercion (`coerceToDelta`), outline preview text (`extractDeltaPreviewText`), media-embed diffing, strip of format attrs (`stripDeltaFormatAttributes`), non-whitespace counting for course validation (matches BE `CountDeltaNonWhitespace`), `countDeltaCodePoints` (plain extract + `unicodeCodePointLength`; matches BE `CountDeltaRunes` for instructor `bio`), and `sanitizeLockedTextDelta` (coerce + keep **string inserts only** + strip `TEXT_DELTA_LOCKED_FORMAT_ATTRIBUTES` = `font`/`size`/`header` — the **single** locked-text policy constant shared by `DeltaEditor` normalize/`text-change` and instructor form hydrate).
- **Defensive parse**: `coerceToDelta` / `parseDelta` always return a valid `DeltaShape`. When JSON has an `ops` array, each op must be a plain object with a valid `insert` (string, or known media embed `{image|video|document: string}`); malformed entries (`null`, numbers, `{}`, unknown embed objects) are **dropped**, never passed to Quill or helpers that read `op.insert`. Empty result after filtering → `createEmptyDelta()`. This prevents legacy DB rows (accepted by older BE counters that skipped non-string inserts) from crashing form hydrate / `DeltaViewer`.
- **Scope**: `DeltaEditor`, `DeltaViewer`, course editor state, outline tab previews, Zod schemas, become-instructor `bio`.
- **Dependencies**: `media.ts` (`MediaEmbedKind` for `DeltaMediaEmbed`); `unicode-length.ts` for `countDeltaCodePoints`.
...
- **Purpose**: Word-like WYSIWYG editing and read-only rendering for Quill Delta JSON. Toolbar: optional font family (unless `lockSystemFont`), text formatting, optional **link** (`allowLink`), configurable media embeds via `mediaEmbedKinds` (toolbar + `MediaCollectionDialog`, paste, drag-and-drop). Embeds show **×** remove; removal calls `onDelete`. Paste/drop delegates upload to parent `onObjectEmbedded`.
- **Props**: `allowMediaEmbed` (default `true`). `mediaEmbedKinds` (default `["image","video"]`; INFO tab uses module-level `ABOUT_COURSE_MEDIA_EMBED_KINDS` = `["image","document"]`; TEXT sub-lesson dialog uses `TEXT_SUB_LESSON_MEDIA_EMBED_KINDS` = `["image"]` in `src/lib/utils/media.ts` — **must be a stable reference**, not an inline array, to avoid Quill re-init on every parent re-render). `allowLink` (default `false`; links on selected text or **image embeds** only). `lockSystemFont` (default `false`; when true: omit font picker **and heading picker** / `font`+`header` formats, strip `font`/`size`/`header` on normalize + onChange via shared `TEXT_DELTA_LOCKED_FORMAT_ATTRIBUTES`, CSS `font-family: inherit` / `--font-sans`). `placeholder` override. `onObjectEmbedded`, `onDelete`. Quill init `useEffect` deps: `[editorFormats, toolbarContainer]` only — use `tRef` inside Quill callbacks; init reads `valueRef.current` (not mount-time snapshot) so link edits survive parent re-renders. **Image embed resize**: 4-corner drag handles (NW/NE/SW/SE) when `mediaEmbedKinds` includes `"image"`; persists `width`/`height` on image blot via `bindQuillImageResize`. **Link text color**: toolbar `linkColor` picker (chain + color bar icon) immediately right of Link button when `allowLink`; palette in `LINK_COLOR_PALETTE`.
...
- **Scope**: Course basic info (`about_course` — image + document + link), section/lesson dialogs (text-only; `min-w-0 overflow-x-hidden` shell matches sub-lesson dialog so long Delta text wraps instead of horizontal scroll), TEXT sub-lessons (image embeds only — no video), instructor become-instructor `bio` (`allowMediaEmbed={false}`, `lockSystemFont`, no media handlers; form hydrate runs `sanitizeLockedTextDelta`), admin `InstructorProfileViewDialog` bio via `DeltaViewer` (`value`/`className` only — `coerceToDelta` defensive-normalizes malformed legacy ops before Quill; new writes are BE allow-listed via `validateBioDelta`; the viewer itself has no restriction props).
...
- **Purpose**: Count / truncate by Unicode **code points** (`Array.from`), matching BE `utf8.RuneCountInString` / `CountRunes`. Prefer over `String.length` (UTF-16 code units) for instructor `teaching_content_ideas` (50–500). For instructor `bio`, count via `countDeltaCodePoints` (plain extract from Delta) instead of applying this helper to the raw Delta JSON string.
- **Scope**: Become-instructor Zod schema + application form counters / input caps for plain-text fields (`teaching_content_ideas`); building block for `countDeltaCodePoints` on `bio`.
- **Dependencies**: none.
...
- **Type**: Fetch policy executor backed by raw/authenticated Xior lifecycle instances
- **Path**: `src/api/xior/client.ts` + `src/api/core/fetch-core.ts`
- **Purpose**: Xior 0.8.3 supplies the HTTP lifecycle over Next.js Fetch through a shared executor factory. Request interceptors resolve runtime headers and preserve native Fetch config; response interceptors preserve success/non-2xx response metadata. Fetch-core is the single project policy owner for `prepareFetchCoreRequest` → `dispatchFetchResponse` → `toFetchCoreOutcome`, including trusted-origin redirect hops, body replay/gzip and typed error metadata. **Do not change public signatures** when shortening internals.
- **Scope**: `api-transport` (`runAttempt`), `raw-http`, `serverRawFetch`, `executeFetchCore` wrapper.
- **Dependencies**: `xior` (no plugins), `fetch-core-body`, `fetch-helpers`, `fetch-error`, `isServer`.
...
- **Purpose**: Browser authenticated transport that creates an isolated Xior executor per request. The Xior request interceptor resolves server Bearer headers from the runtime; its response interceptor preserves typed HTTP metadata. `ApiTransport` retains refresh eligibility, one retry and the reporter matrix (`reportApiError` → sanitized server process log; browser `[API]` Console only when `NODE_ENV === "development"`; browser Zustand always for abnormal). Refresh failure attaches a sanitized cause; browser refresh uses absolute same-origin `/api/auth/refresh`. Default timeout 10s; per-request `options.timeout` (ms) overrides transport/`fetch-core` default (e.g. media upload 30s).
