...
- **Scope**: Course basic info (`about_course` — image + document + link), section/lesson dialogs (text-only; `min-w-0 overflow-x-hidden` shell matches sub-lesson dialog so long Delta text wraps instead of horizontal scroll), TEXT sub-lessons (image embeds only — no video).
