...
2. About you — **`bio` required** (`DeltaEditor` with `allowMediaEmbed={false}`, `lockSystemFont`, `allowLink={false}`; REUSE i18n `course.editor.deltaEditor.placeholderTextOnly`). `lockSystemFont` removes the font picker **and the heading (H1–H3) picker**; `font`/`size`/`header` attributes are stripped on normalize and `text-change` (shared policy `TEXT_DELTA_LOCKED_FORMAT_ATTRIBUTES`). Value is Quill Delta JSON string (**100–2000 Unicode code points** on extracted plain text via `countDeltaCodePoints` / `extractPlainText` + `unicodeCodePointLength`). Live counter on plain extract; **no live-truncate** of Delta — over-limit input **stays in form state** (mirror course `about_course`) and is blocked at Zod submit validation + BE. Prefilled from `latest_submission.profile.bio` on resubmit through `sanitizeLockedTextDelta` (defensive `coerceToDelta` drops malformed ops such as `null`/`42`/`{}`/unknown embeds; keeps string inserts only; strips `font`/`size`/`header`), so an unedited resubmit sends a clean Delta even when the stored value predates the policy or contains legacy malformed ops that older BE counters skipped. BE validates with `CountDeltaRunes` (same code-point unit — **not** UTF-8 bytes) and rejects non-conforming Delta structurally (`validateBioDelta`: string inserts only; attributes limited to `bold`/`italic`/`underline`/`strike`/`list`).
3. Teaching content ideas — **`teaching_content_ideas` required** (`Textarea`, **50–500 Unicode code points**, live counter). Concrete class/course ideas and outlines (Skillshare-style), **not** topic-group taxonomy (section 7). Helper copy clarifies applicants must describe specific class ideas, not only topic categories. Prefilled from `latest_submission.profile.teaching_content_ideas` on resubmit. Same **Unicode code-point unit** as `bio`, but plain-string helpers (`CountRunes` / `unicodeCodePointLength` on the raw field — not Delta).
...
| File | Usage | Width |
|------|-------|-------|
| `become-instructor-application/taxonomy-select.tsx` | Section 7 topic/skill pickers | Full section width |
