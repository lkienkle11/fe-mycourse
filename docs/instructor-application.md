...
2. About you — **`bio` required** (`Textarea`, **100–2000 Unicode code points**, live counter via `unicodeCodePointLength`). Prefilled from `latest_submission.profile.bio` on resubmit. BE validates with `CountRunes` / `utf8.RuneCountInString` after trim (same unit — **not** UTF-8 bytes).
3. Teaching content ideas — **`teaching_content_ideas` required** (`Textarea`, **50–500 Unicode code points**, live counter). Concrete class/course ideas and outlines (Skillshare-style), **not** topic-group taxonomy (section 7). Helper copy clarifies applicants must describe specific class ideas, not only topic categories. Prefilled from `latest_submission.profile.teaching_content_ideas` on resubmit. Same code-point unit as `bio` on FE Zod + BE.
...
**Sidebar required checklist (no headline):** bio (≥100 Unicode code points), teaching content ideas (50–500 Unicode code points), job title + company, years of experience, CV (PDF), ≥1 topic, ≥1 skill.
