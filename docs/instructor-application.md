...
_Last audited: 2026-07-05 — duplicate-certificate rejection on FE Zod (`instructorApplicationSubmitSchema` `.superRefine` emitting `validation.certDuplicate` on **both** colliding rows) and BE (`ErrDuplicateCertificate`, dedicated API code `2010`); composite key normalized case-insensitively with internal whitespace collapsed; editing any certificate field re-runs `refreshCertificateFieldErrors` for the whole section to clear stale sibling-row errors._
