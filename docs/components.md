...
`src/components/features/course/` — course editor tabs and dialogs. `course-editor-basic-tab` — `react-hook-form + zodResolver` over `courseBasicInfoSchema`, with `RequiredLabel`, `FieldError`, and `toastValidationError` on invalid submit (`course.validation.*`); receives grouped `state`, `taxonomyRows`, and `actions` props from the editor shell. `course-editor-outline-tab` — sortable outline surface with grouped `actions` for section/lesson/sub-lesson mutations. `course-editor-collaborators-tab` — `RequiredLabel` on instructor picker and grouped `state`/`data`/`actions` props. `course-editor-dialogs` — `RequiredLabel` on section/lesson/sublesson/quiz/video fields. Outline/collaborator dialogs: pre-submit `toastValidationError` via `useCourseEditorState`; API: `toastApiError`.
