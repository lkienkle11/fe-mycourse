...
`src/components/features/course/` — course editor tabs and dialogs (see below) plus sysadmin catalog UI: `buildCourseAdminListColumns` (shared `DataTable` columns for All/Trash/Review lists), `CourseAdminTableActionsMenu` (shared ⋮ `DropdownMenu` trigger for admin course `DataTable` row actions), `CourseReviewRowActions` (review queue menu: preview for sysadmin, approve, reject), `CourseEditorReviewHistoryTab` (instructor editor tab — paginated review history with filter/URL sync via `useCourseReviewHistory`), `CourseCollaboratorsTab` (paginated collaborator list + search/URL sync via `useCourseCollaborators`; owner with **P67** opens `CourseCollaboratorPickerDialog` via `PermissionGate` + `useCourseInstructorCandidates`), `course-admin-all-page`, `course-admin-trash-page`, and `course-review-page` screens (⋮ menu per row). `course-editor-basic-tab` — `react-hook-form + zodResolver` over `courseBasicInfoSchema`, with `RequiredLabel`, `FieldError`, and `toastValidationError` on invalid submit (`course.validation.*`); receives grouped `state`, `taxonomyRows`, and `actions` props from the editor shell. `course-editor-outline-tab` — sortable outline surface with grouped `actions` for section/lesson/sub-lesson mutations; per-row mutations use shared `CourseOutlineRowActions` (`DropdownMenu` trigger label `course.common.actions`; config keyed by `CourseOutlineItemKind`: `section` | `lesson` | `item`). `SortableList` `onReorder` delegates to `useCourseOutlineReorder` handlers (optimistic SWR outline patch, then reorder API). Description/summary previews use `extractDeltaPreviewText`. `course-editor-collaborators-tab` — `RequiredLabel` on instructor picker and grouped `state`/`data`/`actions` props. `course-editor-dialogs` — shared `CourseOutlineItemDialog` for section/lesson (title + `DeltaEditor` with `allowMediaEmbed={false}`; `min-w-0 overflow-x-hidden` dialog shell so long description/summary wraps); `CourseSubLessonDialog` renders kind-specific fields via `SUB_LESSON_KIND_FIELD_RENDERERS` map (`VIDEO` / `TEXT` / `QUIZ` sub-components) mounted as JSX (`<Renderer {...props} />` in `SubLessonKindFields` — required so per-kind hooks stay in separate component boundaries when the kind `Select` changes); `SubLessonTextFields` passes `mediaEmbedKinds={TEXT_SUB_LESSON_MEDIA_EMBED_KINDS}` (image only); sr-only `DialogDescription` on outline/sub-lesson dialogs; `min-w-0 overflow-x-hidden` + `break-all` on long video IDs/URLs (no horizontal scroll). Outline/collaborator dialogs: pre-submit `toastValidationError` via `useCourseEditorState`; API: `toastApiError`.
