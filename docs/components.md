...
`src/components/features/course/` — course editor tabs and dialogs. `course-editor-basic-tab` — `react-hook-form + zodResolver` over `courseBasicInfoSchema`, with `RequiredLabel`, `FieldError`, and `toastValidationError` on invalid submit (`course.validation.*`); receives grouped `state`, `taxonomyRows`, and `actions` props from the editor shell. `course-editor-outline-tab` — sortable outline surface with grouped `actions` for section/lesson/sub-lesson mutations; description/summary previews use `extractDeltaPreviewText`. `course-editor-collaborators-tab` — `RequiredLabel` on instructor picker and grouped `state`/`data`/`actions` props. `course-editor-dialogs` — shared `CourseOutlineItemDialog` for section/lesson (title + `DeltaEditor` with `allowMediaEmbed={false}`); sub-lesson/quiz/video fields keep `RequiredLabel` / full `DeltaEditor` for TEXT items. Outline/collaborator dialogs: pre-submit `toastValidationError` via `useCourseEditorState`; API: `toastApiError`.
