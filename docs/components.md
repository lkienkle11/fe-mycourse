...
`src/components/features/instructor/` — instructor management + instructor route glue: `InstructorProfileViewDialog`, `InstructorRosterPickerDialog` (multi-select add via `UserMultiSelectPickerDialog`), `InstructorApprovalActions` (`RequiredLabel` on reason, `toastApiError`), `renderInstructorCourseEditorRoute` (shared renderer reused by the 5 course editor route pages). See `docs/instructor-admin.md`.
