...
| `HeaderMobileBar` | `header-mobile-bar.tsx` | Client | Below `lg`: `BrandLogoLink` + burger opening `HeaderMobileSidebar`. |
| `BrandLogoLink` | `brand-logo-link.tsx` | Client | Logo-only link to `homeHref`, no title text. Shared by `HeaderMobileBar` and `DashboardLayout`'s denial-branch header (`className="md:hidden"` there, since `HeaderDashboard`'s own logo+title button already covers `md+`). |
...
**`header/index.ts` barrel:** `BecomeInstructorPromoBanner`, `BrandLogoLink`, `HeaderBrowseNav`, `Header`, `HeaderDashboard`, `HeaderMobileBar`, `HeaderMobileSidebar`, `SidebarAuthFooter`.
...
| Component | File | Type | Description |
|-----------|------|------|-------------|
| `DashboardLayout` | `dashboard-layout.tsx` | Client | Core dashboard shell: `SidebarProvider` + `HeaderDashboard` (`leading` burger `md:hidden`, `trailing` `DashboardHeaderLocale` on `lg+`) + left `Sidebar` (`!top-16`, `h-[calc(100svh-4rem)]`) + `SidebarInset` / `main`. Filters `items` via `useFilteredDashboardItems`; gate via `permissions` / `isAuthorized`; loading uses `SidebarMenuSkeleton`. When the gate denies access, renders `StatusErrorPage` (`forbidden` if `useGetMe().me` is set, `unauthorized` otherwise) beside `HeaderDashboard` and `LoginSignupPopup` instead of the dashboard content; on denial, `HeaderDashboard`'s `leading` slot below `md` renders the shared `BrandLogoLink` (logo only, no title) instead of the authorized branch's burger, since there is no sidebar to open, so the mobile header is never empty. The shell now owns the route-driven dashboard page header (breadcrumb → title → description → actions) by combining `usePathname()`, `resolveDashboardPageHeaderMetadata`, and page-level override state. |
...
| `DashboardUnauthorized` | `dashboard-unauthorized.tsx` | Client | **Deprecated, unused.** Former compact access-denied message; `DashboardLayout` now renders `StatusErrorPage` directly for a permission denial. Kept for reference only. |
...
| `LoginSignupPopup` | `auth/login-signup-popup.tsx` | Client | Full-viewport centered dialog (`z-300`/`z-301`); card `max-w-200`; close button on card (`DialogClose`). **A11y:** `DialogTitle` + `DialogDescription` (sr-only when visual title is elsewhere) must live **inside** `DialogContent` — never as siblings of Content under `Dialog` root (avoids Radix missing Description/Title warnings). |
| `LoginSignupLayout` | `auth/login-signup-layout.tsx` | Client | `lg+` unchanged two-column layout; below `lg` form-only full width. |
| `LoginContent` | `auth/login-content.tsx` | Client | Login form → `loginAction` → **`mutateMe()`** via `useGetMe()`. Submit button shows shared `Spinner` while `isSubmitting`; resend confirm uses `registerAction`. **Forgot password:** `Link` to `forgotPasswordHref` kept as **commented** usage (route constant stays); temporary non-navigating label until `/forgot-password` page exists (avoids Next prefetch 404). |
...
| `StatusErrorPage` | `status-error-page.tsx` | Full-page state for a classified API failure (`variant`: `unauthorized` \| `forbidden` \| `server-error` \| `network`), generalized from `NotFoundPage`'s layout. Optional `title`/`description`/`action` overrides, plus a `fillViewport` flag for full-page (vs. embedded) sizing. Paired with `classifyApiError` (`src/lib/utils/api-error.ts`, table-driven — see `HTTP_STATUS_RULES`) — a 404 response is out of scope and resolves to `"unknown"`. See `docs/screens.md` §Status Error Page. |
...
`src/components/features/instructor/` — … `InstructorProfileViewDialog` (bio via `DeltaViewer`; LinkedIn / GitHub / portfolio URLs as clickable external links; … certificates via `InstructorCertificateCarousel`), … feature model in `src/lib/instructor-application/` (`mergeInstructorApplicationDetail`, `url-validation.ts`, `search-text.ts`, `validate-application-form.ts` incl. `refreshCertificateFieldErrors` on certificate row delete …). Shared carousel: `src/components/ui/carousel.tsx` (Embla). …
