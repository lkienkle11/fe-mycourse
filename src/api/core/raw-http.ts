...
export type RawMutationApiOptions = RawApiOptions & {
   ...
   * CompressionStream("gzip"). FormData and non-JSON bodies are never compressed.
   * No caller enables this yet (BE does not decompress gzip).
   */
  compress?: boolean;
};
...
async function rawRequest<T>(
  ...
  options: RawApiOptions | RawMutationApiOptions,
): Promise<ApiResult<T>> {
  const {
    headers,
    cookies,
  ...
  const compress =
    method === "POST" || method === "PUT" || method === "PATCH"
      ? (options as RawMutationApiOptions).compress
      : undefined;
