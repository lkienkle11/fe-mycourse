...
function prepareFetchCoreRequest(init: FetchCoreInit): PreparedFetchCore {
  ...
  const body =
    init.mode === "authenticated"
      ? buildAuthenticatedBody(init.data, allowBody, request)
      : buildRawBody(init.data, allowBody, request);
...
export async function executeFetchCoreOutcome<T>(
  ...
  const prepared = prepareFetchCoreRequest(init);
  try {
