    ...
    <section className="py-20 bg-background border-t">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
          ...
          <Button size="lg" variant="secondary" className="font-medium bg-white text-primary hover:bg-slate-100 hidden sm:flex">
            {t("allWebinars")}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {webinars.map((webinar) => (
            <div key={webinar.id} className="relative group bg-card rounded-2xl border p-6 flex flex-col hover:shadow-lg transition-all hover:border-primary/30">
              <div className="flex gap-2 flex-wrap mb-4">
                {webinar.tags.map(tag => (
                  <Badge key={tag} variant="secondary" className="bg-primary/5 text-primary hover:bg-primary/10 border-none font-medium">
                    {tag}
                  </Badge>
                ))}
              </div>
              ...
              <div className="mt-auto space-y-4">
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-muted-foreground mr-1">{t("hostedBy")}</span> {webinar.host}
                  </div>
                  </div>
